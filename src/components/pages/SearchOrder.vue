    ...
    <div class="container main-contant py-3 mt-5">
      ...
      <div class="form-group d-flex justify-content-center align-items-center ml-auto mr-auto">
        ...
        <button
          ...
          :class="{'disabled': status.search}"
        >
          搜尋
          <i class="fas fa-spinner fa-spin" v-if="status.search"></i>
        </button>
      </div>
      <!--search end-->
      <section class="row justify-content-center px-4 px-md-0" v-if="products.length != 0">
        <div class="col-md-10 col-lg-8">
          ...
          <div class="text-center font-weight-bold">訂單編號：{{orderID}}</div>
...
export default {
  ...
  methods: {
    search() {
      ...
      }/order/${vm.searchItem.trim()}`;
      this.$http.get(api).then(response => {
        vm.status.search = false;
        if (response.data.success) {
          (vm.searchItem = ""), (vm.orderID = response.data.order.id);
